...
namespace Shop.Infrastructure.Persistent.Ef.CategoryAgg
{
    internal class CategoryRepository : BaseRepository<Category>, ICategoryRepository
    {
        ...
        {
            var category = await Context.Categories.Include(f=>f.Childs)
                .ThenInclude(f=>f.Childs).FirstOrDefaultAsync(f => f.Id == categoryId);
            if(category == null)
            {
                return false;
            }

            var isExistProduct = await Context.Products.AnyAsync(f=>f.CategoryId == categoryId || f.SubCategoryId == categoryId || f.SecondarySubCategoryId == categoryId);
